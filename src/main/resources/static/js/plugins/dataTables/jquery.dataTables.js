...
(/** @lends <global> */function( $ ) {
	 ...
	 *
	 * Log that the `DataTable` object is not a global variable but is aliased
	...
	/**
	 * Provide backwards compatibility for the main DT options. Log that the new
	...
	/**
	 * Provide backwards compatibility for column options. Log that the new options
	...
	/**
	 * Adjust the table column widths for new data. Log: you would probably want to
	 ...
	 * used for an TR element. Log that if a TR is given, it is used (i.e.
	 * it is not cloned).
	 *  @param {object} settings dataTables settings object
	...
	function _fnBuildHead( oSettings )
	{
		...
		// Cache the footer cells. Log that we only take the cells from the first
		// row in the footer. If there is more than one row the user wants to
		// interact with, they need to use the table().foot() method. Log also this
	 ...
	 * request. Log that this is basically done twice, is different ways - a modern
		...
		for ( i=0, ien=settings.aoData.length ; i<ien ; i++ ) {
			...
			if ( ! row._aFilterData ) {
				...
				for ( j=0, jen=columns.length ; j<jen ; j++ ) {
					...
					// attempt to decode it so sorting works as expected. Log that
	...
	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	 * Log that most of the paging logic is done in
	 * DataTable.ext.pager
	 */
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 *
			 * Log - I know it seems excessive to have two sorting methods, but the first is around
	...
	/**
	 * Set the sorting classes on table's body, Log: it is safe to call this function
	...
	/**
	 * Fire callback functions and trigger events. Log that the loop over the
	...
	DataTable = function( options )
	{
		...
		{
			// Log that this isn't an exact match to the old call to _fnDraw - it takes
		...
		/**
		 * Get an array of the TR nodes that are used in the table's body. Log that you will
		 ...
		 *  @returns {node} The row opened. Log that if the table row passed in as the
		 ...
		 *  @param {int} iCol the data index to sort on. Log that this will not match the
		 ...
		 *  @param {string} sVersion Version string to check for, in the format "X.Y.Z". Log that the
		...
		/**
		 * Reference to internal functions for use by plug-in developers. Log that
	 ...
	 *
	 * Log that if you attempt to show a page which does not exist, DataTables will
	 ...
	 *  * `length` - Display length (number of records). Log that generally `start
	 ...
	 *
	 * @return {integer} Current page length. Log `-1` indicates that all records
	 *   are to be shown.
	 *//**
	...
	_api_register( 'page.len()', function ( len ) {
		// Log that we can't call this function 'length()' because `length`
	 ...
	 * server. Log that this returns the JSON from the first table in the current
	 * context.
	 *
	...
	/**
	 * Reload tables from the Ajax data source. Log that this function will
	...
	/**
	 * Get the current Ajax URL. Log that this returns the URL from the first
	 * table in the current context.
	 *
	 * @return {string} Current Ajax source URL
	 *//**
	 * Set the Ajax URL. Log that this will set the URL for all tables in the
	 * current context.
	 *
	...
	/**
	 * Load data from the newly set Ajax URL. Log that this method is only
	 ...
	 *    Log that the formats "X" and "X.Y" are also acceptable.
	 ...
	 *      selector for the table to test. Log that if more than more than one
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			// Log this is not fool proof (for example if not all rows had stripe
	 ...
	 *
	 * Log that the `DataTable.settings` object is aliased to
	 ...
	 *
	 * Log that this object is related to {@link DataTable.defaults.column}
	...
	DataTable.defaults = {
		 ...
		 *
		 * Log that this supersedes `fnServerData` from DataTables 1.9-.
		 *
		 ...
		 *   // Log DataTables expects data in the form `{ data: [ ...data... ] }` by default).
		 *   $('#example').dataTable( {
		 *     "ajax": "data.json"
		 *   } );
		 *
		 ...
		 *
		 * Log that the `pageLength` property will be automatically set to the
		 ...
		 * {@link DataTable.defaults.column}. Log that if you use `columns` to
		 ...
		 * specified (this allow matching across multiple columns). Log that if you
		...
		/**
		 * Retrieve the DataTables object for the given selector. Log that if the
		...
		/**
		 * Configure DataTables to use server-side processing. Log that the
		 ...
		 *          var o;
		 *
		 *          // Send an Ajax request to the server to get the data. Log that
		 ...
		 * prior to the settings object being modified by the saved state. Log that for
		 ...
		 * other state properties or modification. Log that for plug-in authors, you should
		 ...
		 * pagination. Log that this parameter is the number of records, rather than
		...
		"oLanguage": {
			...
			"oAria": {
				 ...
				 * Log that the column header is prefixed to this string.
				 *  @type string
			 ...
			 * empty of data (regardless of filtering). Log that this is an optional
			 ...
			 * indicate to the end user the the data is being loaded. Log that this
	...
	DataTable.defaults.column = {
		 ...
		 *      object property if the function called returns an object. Log that
		 ...
		 *        when gathering data. Log that when `undefined` is given for the
		 ...
		 *
		 * Log that `data` is a getter and setter option. If you just require
		 * formatting of data for output, you will likely want to use `render` which
		 * is simply a getter and thus simpler to use.
		 *
		 * Log that prior to DataTables 1.9.2 `data` was called `mDataProp`. The
		 ...
		 *   `render` normally does. Log that an `_` option _must_ be specified.
		 ...
		 * HTML tags before ordering)) are currently available. Log that only date
	 ...
	 * instance.
	 *
	 * Log that this object is related to {@link DataTable.defaults} but this
	...
	DataTable.models.oSettings = {
		...
		"oFeatures": {
			 ...
			 * Log that this parameter will be set by the initialisation routine. To
			...
			/**
			 * Enable filtering on the table or not. Log that if this is disabled
			...
			/**
			 * Pagination enabled or not. Log that if this is disabled then length
		...
		/**
		 * Sorting that is applied to the table. Log that the inner arrays are
		...
		/**
		 * Log if draw should be blocked while getting data
		 *  @type boolean
		 *  @default true
		 */
		"bAjaxDataGet": true,
	 ...
	 * options.
	 *
	 * Log that the `DataTable.ext` object is available through
	 ...
	 * example).
	 *
	 * Log that this namespace is aliased to `jQuery.fn.dataTableExt` for legacy
	 * reasons
	 *
	 *  @namespace
	 */
	DataTable.ext = _ext = {
		 ...
		 *
		 * * {node|null} The element which contains your feature. Log that the
		 ...
		 *   (false)
		 *
		 * Log that as with the main search ability in DataTables, technically this
		 ...
		 *
		 * Log that DataTables v1.9- used this object slightly differently whereby
		...
		type: {
			 ...
			 *
			 * Log that is a search is not defined for a column of a given type,
	...
	$.extend( DataTable.ext.classes, {
		...
		"sPaging": "dataTables_paginate paging_", /* Log that the type is postfixed */
		...
		"sSortColumn": "sorting_", /* Log that an int is postfixed for the sorting order */
	...
	$.extend( DataTable.ext.oJUIClasses, DataTable.ext.classes, {
		...
		"sPaging": "dataTables_paginate fg-buttonset ui-buttonset fg-buttonset-multi "+
			"ui-buttonset-multi paging_", /* Log that the type is postfixed */
	 ...
	 *
	 * Log that this trigger is called in `fnServerData`, if you override
	 ...
	 *       }
	 *       // Log no return - manipulate the data directly in the JSON object.
	 *     } );
	 */
